...
RSpec.describe Git::Cop::Styles::CommitTrailerCollaboratorEmail do
  subject(:commit_trailer_collaborator_email_style) { described_class.new commit: commit }
  ...
  describe "#valid?" do
    context "with no matching key" do
      ...
      it "answers true" do
        expect(commit_trailer_collaborator_email_style.valid?).to eq(true)
      end
    end
    ...
    context "with missing '@' symbol" do
      ...
      it "answers false" do
        expect(commit_trailer_collaborator_email_style.valid?).to eq(false)
      end
    end
  ...
  describe "#issue" do
    let(:issue) { commit_trailer_collaborator_email_style.issue }
